-- Recreate admin_create_account function after account_type enum change
-- This ensures the function works correctly with the new 'current' account type
...
BEGIN
  ...
  RETURNING
    accounts.id, accounts.user_id, accounts.name, accounts.type, accounts.balance, 
    accounts.account_number, accounts.status, accounts.account_state, 
